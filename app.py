            ...
            for line in target_lines:
                if not author_found:
                    ...
                    if any(kw in line.lower() for kw in designation_kw):
                        ...
                        title_lines.append(line) # இல்லையென்றால் அது தலைப்பு (Title)
                else:
                    author_lines.append(line)
                    
            # கண்டுபிடிக்க முடியவில்லை என்றால், முதல் பாதியைத் தலைப்பாகவும், மீதியைப் பெயராகவும் எடுக்கிறோம்
            if not author_found and len(target_lines) > 1:
                title_lines = target_lines[:-1]
                author_lines = [target_lines[-1]]
                
            # தலைப்பை முழுமையாக இணைத்தல் (இரண்டு வரிகளில் இருந்தாலும் ஒன்றாக மாறிவிடும்)
            title = " ".join(title_lines)
            
            # 4. அசல் பெயரை மட்டும் வெட்டி எடுத்தல் (கமாவுக்கு முன் உள்ளதை மட்டும்)
            author_str = author_lines[0] if author_lines else "Name Not Found"
            author_str = author_str.split(',')[0].strip()
            
            # இரண்டு ஆசிரியர்கள் இருந்தால் பிரிப்பது (and அல்லது & வைத்து)
            authors_list = [a.strip() for a in re.split(r'\band\b|&', author_str) if a.strip()]
